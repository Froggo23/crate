-- Fix the return type of the tag-overlap column.
--
-- The 0.0 / 1.0 literals inside the CASE resolve to `numeric`, while the
-- function declares `double precision`, so every call failed with
-- 42804 "Returned type numeric does not match expected type double precision
-- in column 33". Casting the lateral's output and the distance expression fixes
-- it. Same logic as the previous migration, types corrected.
